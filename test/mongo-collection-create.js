...
var should = require('should'),
    ...
    handler = require('../handlers/mongo-collection-create');

describe('mongo-collection-create', function() {
    beforeEach(function() {
        var self = this;

        this.insert = sinon.spy(function(data, callback) {
            callback(null, data);
        });

        this.collection = { insert: this.insert };
    ...
    it('should use the defined collection', function(done) {
        var self = this;
        handler({ collection: 'foo' })(null, this.$$resolver, this.$resolver, function() {
    ...
    it('should not run the query if an error is defined in the resolver', function(done) {
        var self = this;
        handler({ collection: 'foo' })({}, this.$$resolver, this.$resolver, function() {
    ...
    it('should inject the results if no error occurred and inject is defined', function(done) {
        var self = this;
        handler({ inject: 'ducks' })(null, this.$$resolver, this.$resolver, function() {
    ...
    it('should use the defined resource', function(done) {
        var collection = {
                insert: sinon.spy(function(data, callback) {
                    callback(null, data);
                })
            },
            $mongo = {
        ...
        handler({ resource: $mongo })(null, this.$$resolver, this.$resolver, function() {
    ...
    it('should use `$mongo` if no resource is defined', function(done) {
        var self = this;
        handler({})(null, this.$$resolver, this.$resolver, function() {
    ...
    it('should use the defined source', function(done) {
        var self = this;
        handler({ source: 'bar' })(null, this.$$resolver, this.$resolver, function() {
            self.insert.called.should.equal(true);
            self.insert.calledWith('bar').should.equal(true);
            done();
        });
    });
    ...
    it('should use `$body` if no source is defined', function(done) {
        ...
        handler({})(null, this.$$resolver, this.$resolver, function() {
            self.insert.called.should.equal(true);
            self.insert.calledWith('$body').should.equal(true);
            done();
        });
    });
    ...
    it('should call insert on the collection with the defined data', function(done) {
        var self = this,
            source = { foo: 'foo' };
        handler({ source: source })(null, this.$$resolver, this.$resolver, function() {
            self.insert.called.should.equal(true);
            self.insert.calledWith(source).should.equal(true);
            done();
        });
    });
});
